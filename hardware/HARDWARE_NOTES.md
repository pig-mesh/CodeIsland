...
- `BOOT = GPIO0`（板载 `BOOT` 键，运行时长按调节亮度；开机/复位时按住会进入下载模式）
- `BAT_EN = GPIO2`（电池供电保持，高电平）
- `BAT_ADC = GPIO1`（电池电压分压采样，用于粗略状态灯）
...
- 固件通过 `BAT_ADC/GPIO1` 读取电池分压，按 `3300mV -> 0%`、`4200mV -> 100%` 线性估算电量；右上角状态灯 `<30%` 红色、`<50%` 黄色、`>=50%` 绿色。
- 5 分钟无任务后只是软息屏；BLE 不关闭，新的运行、审批、任务计时或事件帧会自动唤醒屏幕。
- 短按 `PWR` 唤醒屏幕；`PLUS` 业务键仍负责手动唤醒/交互。
- 长按 `PWR` 是电池断电关机，BLE 已断开，不能靠任务事件自动唤醒。
- USB 供电时拉低 `BAT_EN` 不一定会断电，因为 USB 仍在供电。
- 电量百分比只是基于电压的粗略状态提示；充电中或负载变化时会有偏差，不等同于专业电量计。
