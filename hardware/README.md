...
> **关于按钮**：默认使用板载 `PLUS` 键（GPIO4）作为业务按键，无需外接按钮——短按切换吉祥物、长按切换 Demo 模式。`BOOT` 键（GPIO0）长按用于本地亮度循环调节；开机/复位时按住 `BOOT` 仍会进入下载模式。`PWR` 键（GPIO5）只负责电源：电池供电时长按开机，运行中长按关机；短按 `PWR` 不作为唤醒动作。旧 C6 profile 仍使用 `BOOT` 键（GPIO9）作为业务键。
...
5. 屏幕右上角会显示粗略电量状态灯：`<30%` 红色、`<50%` 黄色、`>=50%` 绿色。
6. **PWR 短按不会唤醒屏幕**；Buddy 的业务交互仍使用 **PLUS** 键。
...
7. 在 macOS 端可远程调节屏幕亮度（10%–100%）与翻转方向（朝上 / 朝下），无需重新烧录。
