...
> **关于按钮**：默认使用板载 `PLUS` 键（GPIO4）作为业务按键，无需外接按钮——短按切换吉祥物、长按切换 Demo 模式。`PWR` 键（GPIO5）只负责电源：电池供电时长按开机，运行中长按关机；短按 `PWR` 不作为唤醒动作。旧 C6 profile 仍使用 `BOOT` 键（GPIO9）。
...
4. **PWR 短按不会唤醒屏幕**；Buddy 的业务交互仍使用 **PLUS** 键。
